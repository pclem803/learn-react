...
const LoadItems = ({ products, cartState, openState, stockState, userState }) => {
  return (
    <React.Fragment>
      <Column.Group breakpoint="desktop" multiline gapSize={6}>
        {products.map(this_product => (
          <Column size={3} textAlign="centered">
            <Box color="primary" size={20}>
              <Content>{this_product.title}</Content>
              <GetImage code={this_product.sku} />
              <MakeDesc code={this_product} />
              <MakeButtons product={this_product} cartState={cartState} openState={openState} stockState={stockState} userState={userState}/>
            </Box>
          </Column>
        ))}
      </Column.Group>
